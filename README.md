# Computing from the Ground Up

A book that explains how computers work — starting from a single wire and building up to a CPU, one idea at a time.

**Read it at:** https://mobiledge.github.io/computing-from-the-ground-up/

## Contents

1. The Wire: Electricity as Information
2. Making Decisions with Electricity
3. Teaching a Wire to Remember *(coming soon)*
4. Scaling Memory Up *(coming soon)*
5. Teaching Wires to Calculate *(coming soon)*
6. Directing Traffic: The Control Unit *(coming soon)*
7. Putting It Together: A Simple CPU *(coming soon)*

## Development

Built with [Hugo](https://gohugo.io) and the [hugo-book](https://github.com/alex-shpak/hugo-book) theme.

```bash
hugo server   # preview locally at http://localhost:1313
```

Chapters live in `content/docs/`. Each file requires frontmatter:

```yaml
---
title: "Chapter Title"
weight: 3
---
```

Pushing to `main` deploys automatically via GitHub Actions.
